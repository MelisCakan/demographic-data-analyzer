...
def calculate_demographic_data(print_data=True):
    ...
    average_age_men = round((df.age[df.sex == 'Male'].sum() / df.sex.value_counts()["Male"]).item(), 1)

    # What is the percentage of people who have a Bachelor's degree?
    percentage_bachelors = round((df.education.value_counts()['Bachelors'] / df.education.count() * 100).item(), 1)
    ...
    percentage_by_country = (rich_people_by_country / total_people_by_country * 100)
    highest_earning_country = percentage_by_country.idxmax()
    highest_earning_country_percentage = round(percentage_by_country.max().item(), 1)
